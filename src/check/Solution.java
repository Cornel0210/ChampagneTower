...
public class Solution {
    ...
    public static double champagneTower(int poured, int query_row, int query_glass) {
        ...
        List<Double> previousRow = new ArrayList<>(Arrays.asList((double) poured));

        int row = 1;
        int glasses = 1;
        while (row <= query_row) {
            List<Double> currentRow = new ArrayList<>(glasses + 1);
            currentRow.add(Math.max(((previousRow.get(0) - 1) / 2), 0));
            for (int i = 1; i < glasses; i++) {
                currentRow.add(Math.max(
                        (Math.max(((previousRow.get(i - 1) - 1) / 2), 0) + Math.max(((previousRow.get(i) - 1) / 2), 0)), 0));
            }
            currentRow.add(Math.max(((previousRow.get(previousRow.size()-1) - 1) / 2), 0));
            if (row == query_row){
        ...
        return previousRow.get(0) >= 1 ? 1 : previousRow.get(0);
    }
}
